...
export default {
    ...
    computed: {
        ...
        setList () {
            ...
            let temp = [] 
            let list = []
            let count = Math.floor(this.pageInfo.pagegroup / 2) 
            let center = this.pageInfo.current
            ...
            if(this.pageInfo.current > this.page - count) { center = this.page - count }
            ...
            if(this.page > this.pageInfo.pagegroup) {
                if(this.pageInfo.current > count + 1) { list.unshift({ text:'...', val: list[0].val - 1 }) }
                if(this.pageInfo.current < this.page - count) { list.push({ text:'...', val: list[list.length - 1].val + 1 }) }
            }
            return list
        }
    },
